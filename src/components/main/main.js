        ...
        {weatherData !== undefined && weatherData.map((item, index) => (
          <li key={index}>
            <div id={index} className={index === 0 ? "bg-dark card border-white opacity" : "card opacity"}>
              <p className='days-text'>{days[`${new Date(item["dt"] * 1000).getDay()}`]}</p>
              <span>
                <img src={`http://openweathermap.org/img/wn/${item["weather"][0]["icon"]}@2x.png`} alt="" />
              </span>
              <p className='temp'>{Math.round(item.temp.day)}°</p>
              <p className='temp-max-min'> Mak: {Math.round(item.temp.max)}° Min: {Math.round(item.temp.min)}°</p>
            </div>
          </li>
